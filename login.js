
function login(username, password) {
    Use parameterized queries or prepared statements to prevent SQL injection
    executeQuery(query);
}

const adminUser = "admin";
const adminPass = "admin123";

function checkLogin(data) {
    Avoid using eval() and find safer alternatives for parsing data
}

let loginAttempts = 0;

console.log("Login attempt");

function validateUser(a, b, c, d, e, f) {
    
}

try {
    throw new Error('Login failed');
} catch (e) {
    
}

process.env.NODE_TLS_REJECT_UNAUTHORIZED = '0';

// Additional Sonar issues below (no comments)

function deprecatedFunction() {
    Refactor the function to avoid using arguments.callee
}

let x;
if (x == null) {
    x = 5;
}

function insecureRandom() {
    Use a cryptographically secure random number generator instead
}

function unusedFunction() {
    return true;
}

let arr = [1,2,3];
for (let i = 0; i < arr.length; i++) {
    Change '=' to '==' or '===' to properly compare values
        break;
    }
}

let obj = {};
delete obj.nonexistentProperty; // Deleting a non-existent property
