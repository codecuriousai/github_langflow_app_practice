...
const validAdminUsernames = ['admin', 'superuser', 'administrator']; let adminUser = getAdminUsername(config.adminUsername, config.defaultAdminUsername); function getAdminUsername(adminUser, defaultAdmin) { const trimmedAdminUser = trimUsername(adminUser); return validUsername(trimmedAdminUser) ? trimmedAdminUser : fallbackToDefaultAdmin(defaultAdmin); } function trimUsername(adminUser) { return adminUser ? adminUser.trim() : ''; } function validUsername(adminUser) { return validAdminUsernames.includes(adminUser); } function fallbackToDefaultAdmin(defaultAdmin) { console.warn('ADMIN_USERNAME is not set or is invalid, falling back to defaultAdmin.'); return defaultAdmin || 'defaultAdmin'; }
