...
const validAdminUsernames = ['admin', 'superuser', 'administrator']; let adminUser = config.adminUsername; if (!validAdminUsernames.includes(adminUser)) { console.warn('ADMIN_USERNAME is not set or is invalid, falling back to defaultAdmin.'); adminUser = config.defaultAdminUsername || 'defaultAdmin'; } else { adminUser = adminUser.trim(); }
